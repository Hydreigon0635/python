...
class MyApp1(tkinter.Frame):
    ...
    def l_click_canvas(self, event):
        if self.point_l and self.point_r:
            ...
            coross_name_l = ''
            if not self.isNew_l:
                coross_name_l = "cross_" + str(self.cross_num).zfill(3)
                cur.execute(f'''
                ...
                coross_name_l = self.isNew_l['name']

            coross_name_r = ''
            if not self.isNew_r:
                coross_name_r = "cross_" + str(self.cross_num).zfill(3)
                cur.execute(f'''
                    INSERT INTO cross_position VALUES (
                    "cross_{str(self.cross_num).zfill(3)}", {self.point_r[0]}, {self.point_l[1]}
                )''')
                self.cross_num += 1
            else:
                coross_name_r = self.isNew_r['name']

            dist = math.sqrt((self.point_l[0] - self.point_r[0]) ** 2 + (self.point_l[1] - self.point_r[1]) ** 2)
            cur.execute(f'''
